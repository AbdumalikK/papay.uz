<?php
  ...
  if (!empty($_POST['create_account'])) {
    ...
    try {
      ...
      if (database::num_rows(database::query("select id from ". DB_TABLE_CUSTOMERS ." where username = '". database::input($_POST['username']) ."' limit 1;"))) throw new Exception(language::translate('error_email_already_registered', 'The username address already exists in our customer database. Please login or select a different username address.'));
      ...
      $aliases = array(
        ...
        '%customer_username' => $customer->data['username'],
      );
      ...
      $email = new ent_email();
      $email->add_recipient($_POST['username'], $_POST['firstname'] .' '. $_POST['lastname'])
            ->set_subject($subject)
            ->add_body($message)
            ->send();
