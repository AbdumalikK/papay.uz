  ...
  if (!empty($_POST['login'])) {
    ...
    try {
      ...
      if (empty($_POST['email'])) {
        throw new Exception(language::translate('error_missing_login_credentials', 'You must provide ID'));
      }

      $customer_query = database::query(
        "select * from ". DB_TABLE_CUSTOMERS ."
        where lower(username) = lower('". database::input($_POST['email']) ."')
        limit 1;"
      );

      if (!$customer = database::fetch($customer_query)) {
        throw new Exception(language::translate('error_email_not_found_in_database', 'The ID does not exist in our database'));
      }
    ...
    // Compatibility with older passwords (prior to LiteCart 2.2.0)
      //if (substr($customer['password_hash'], 0, 1) != '$') {
        ...
        // if (functions::password_checksum($customer['username'], $_POST['password']) != $customer['password_hash']) {
      ...
      if (!empty($_POST['remember_me'])) {
        $checksum = sha1($customer['username'] . $_SERVER['REMOTE_ADDR'] . ($_SERVER['HTTP_USER_AGENT'] ? $_SERVER['HTTP_USER_AGENT'] : ''));
        header('Set-Cookie: customer_remember_me='. $customer['username'] .':'. $checksum .'; Path='. WS_DIR_APP .'; Expires='. gmdate('r', strtotime('+3 months')) .'; HttpOnly; SameSite=Lax', false);
      }
